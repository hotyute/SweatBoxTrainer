...
int BasicStream::add_data(SOCKET clientSocket)
{
	...
	const std::size_t incomingBytes = static_cast<std::size_t>(bytes_available);

	// Check if we have enough capacity. If not, grow the buffer.
	if (readable + incomingBytes > capacity) {
		// Grow by doubling, or by the required amount if that's larger.
		std::size_t new_capacity = std::max(capacity * 2, readable + incomingBytes);
	...
	const int bytesToRead = static_cast<int>(std::min<std::size_t>(
		incomingBytes,
		static_cast<std::size_t>(std::numeric_limits<int>::max())));
	const int bytes_read = recv(clientSocket, data + readable, bytesToRead, 0);
	...
	readable += static_cast<std::size_t>(bytes_read);
